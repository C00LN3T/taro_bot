...
- **Рефералы:** кнопка «Пригласить друга» создаёт ссылку `https://t.me/<bot_username>?start=ref_<user_id>` с бонусом по умолчанию +5 бесплатных раскладов. Кнопка «Мои рефералы» показывает число приглашённых и текущий баланс бонусов. Админ-команды: `/set_ref_bonus <N>`, `/ref_enable`, `/ref_disable`, `/ref_stats`.

### Развёртывание через вебхуки (например, Render)
Бот поддерживает работу через вебхуки с простым HTTP-сервером на базе `aiohttp`. Установите переменную `USE_WEBHOOK=true` и передайте параметры подключения:

- `WEBHOOK_HOST` — публичный базовый URL (например, `https://example.onrender.com`). Можно использовать `RENDER_EXTERNAL_URL`.
- `WEBHOOK_PATH` — путь обработчика, по умолчанию `/webhook`.
- `WEBHOOK_SECRET` — необязательный секрет для проверки подписи.
- `PORT` или `WEBHOOK_PORT` — порт, который должен слушать сервис (Render передаёт переменную `PORT`).

Если `USE_WEBHOOK` не задана, бот работает в режиме long polling.
